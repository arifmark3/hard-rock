...
const displaySampleAreaHidden = (document.getElementById(
  'display-hidden'
).style.display = 'none');

const searchResultShow = document.querySelector('.search-result-show');

const searchSongBtn = document.getElementById('search-btn');
searchSongBtn.addEventListener('click', function () {
  searchSongBox();
});

function searchSongBox() {
  const searchAreaInput = document.getElementById('search-box').value;
  const summerSongs = searchAreaInput;
